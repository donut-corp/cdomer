...
impl Parser {
    ...
    fn parse_if_stmt(&mut self) -> PResult<Stmt> {
        ...
        let else_branch = if self.check(&TokenKind::Else) {
            ...
            if self.check(&TokenKind::If) {
                // else if -> trata como bloco contendo um if
    ...
    // assignment -> or -> and -> equality -> comparison -> term -> factor -> unary -> postfix -> primary
    ...
    fn parse_primary(&mut self) -> PResult<Expr> {
        ...
        match self.peek().clone() {
            ...
            TokenKind::Ident(name) => {
                self.advance();
                // struct literal: Nome { campo: valor, ... }
    ...
    // Heuristica simples para distinguir `Nome { ... }` (struct literal) de um
    // identificador seguido por um bloco em outro contexto (nao ocorre em
    // expressoes normalmente, mas evitamos colisao com `if (x) { ... }` etc,
    // que nunca chega aqui porque cond de if/while nao captura LBrace como
    // struct lit por estarem dentro de parenteses antes do bloco).
